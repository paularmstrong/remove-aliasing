...
function transformer(file, api, options) {
	...
	const files = fs.readdirSync(rootDir).map((filepath) => filepath.replace(/\.(j|t)sx?$/, ''));
	...
	root.find(j.ImportDeclaration).forEach((nodepath) => {
		...
		let newSource = path.relative(path.join(rootDir, path.dirname(filepath)), path.join(rootDir, source));
		if (/^\s+$/.test(newSource)) {
			newSource = '.';
		}
